...
def model_6_loader(script_list=Config.Effective_Scripts,
    ...
    for story in stories:
        ...
        while x_pointer < len(story):
            ...
            context_ep = list(dict.fromkeys(context_ep))
            #if 'Story_Begin_tion>' in context_ep:
            #    context_ep.remove('Story_Begin_tion>')
            #print (context_ep)
    ...
    for i in range(batch_size):
        for j in range(Config.Context_length):
            ...
            #print (dict_event_to_int)
            #print (e_p_context[i][j])
            #print (i + '   ' + j)
            #if isinstance(e_p_context[i], list):
            #    print ('yes yes yes')
            #    print (len(e_p_context))
            #    print (len(e_p_context[0]))
...
#    x_train = x_int[perm[:int(batch_size * (1 - val_proportion))]]
    ...
    #print("x train inside load data is :")
...
def word_to_int(seq, dict_word_to_int):
    ...
    """
    #print("seq inside word_to_int")
    #print(seq[0])
    #print("seq length is : ")
    #print(len(seq))
    #print("dict_word_to_int inside word_to_int")
    #print(dict_word_to_int)
...
def load_word2vec_matrix(w2v_path=Config.W2V_wrapper_path):
    #print((dill.load(open(w2v_path, 'rb'))).shape)
    return dill.load(open(w2v_path, 'rb'))


def seq_generator(script_list=list(['gender_pay_gap']), courpus_folder=Config.Clean_corpus_6_folder):
    # load corpus
    corpus = list()
    text = list()
    events = list()
